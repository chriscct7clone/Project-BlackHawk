/* Blackhawk Admin Panel - common */
	...
	$(document).ready(function() {
        ...
        if(!is_touch_device()){
		    //* popovers
            blackhawk_popOver.init();
        }
		//* breadcrumbs
        blackhawk_crumbs.init();
		//* pre block prettify
		if(typeof prettyPrint == 'function') {
			prettyPrint();
		}
		//* external links
		blackhawk_external_links.init();
		//* accordion icons
		blackhawk_acc_icons.init();
		//* colorbox single
		blackhawk_colorbox_single.init();
		//* main menu mouseover
		blackhawk_nav_mouseover.init();
		//* top submenu
		blackhawk_submenu.init();
		//* style switcher
		blackhawk_style_sw.init();
	});
	...
	blackhawk_style_sw = {
		init: function() {
			...
			$('.style_switcher .jQclr').click(function() {
				...
				$('#link_theme').attr('href','../assets/css/'+style_selected+'.css');
            });
			...
			$('input[name=ssw_layout]').click(function(){
				...
				$('body').removeClass('blackhawk-fixed').addClass(layout_selected);
			});
			//* menu show
